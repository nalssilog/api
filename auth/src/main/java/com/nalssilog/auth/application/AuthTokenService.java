...
import com.nalssilog.auth.repository.RefreshTokenStore.RotationResult;
import com.nalssilog.auth.repository.RefreshTokenStore.RotationStatus;
...
public class AuthTokenService {
    ...
    public TokenPair refresh(String refreshToken, DeviceInfo device) {
        ...
        return switch (result.status()) {
            case ROTATED -> {
                ...
                yield tokenPair(
                        member, currentSession.provider(), replacementToken, properties.jwt().refreshTokenTtl());
            }
    ...
    public void revoke(String refreshToken) {
        ...
        refreshTokenStore.revokeByTokenHash(tokenHash, properties.jwt().refreshTokenTtl())
                .ifPresentOrElse(
                        ...
                        () -> log.info("auth.refresh.revoke_skipped reason=not_found token={}", fingerprint(tokenHash)));
    }
    ...
    private TokenPair issueWithSession(Long memberId, MemberStatus status, Provider provider, String sessionId,
        ...
        return new TokenPair(
                jwtTokenProvider.createAccessToken(memberId, status, provider),
                refreshToken,
                properties.jwt().refreshTokenTtl());
    }
    ...
    private TokenPair tokenPair(MemberInfo member, Provider provider, String refreshToken, Duration maxAge) {
        return new TokenPair(
                jwtTokenProvider.createAccessToken(member.id(), member.status(), provider),
                refreshToken,
                maxAge);
    }
